import speech_recognition as sr    #takes voice input
import pyttsx3                   # voice output
import time  #displays time
import pywhatkit #for webbrowsing
import pyautogui #to control keyboard keys virtually
import pyjokes   #tell jokes
from comtypes import CLSCTX_ALL  # for volume control
from pycaw.pycaw import AudioUtilities, IAudioEndpointVolume


def sound(n):
    speech = pyttsx3.init()     # initializes pyttsx3 module
    speech.setProperty('rate', 180)     #controlvoice speed
    speech.say(n)           # outputs the voice
    speech.runAndWait()      # waits after delivering the output

def command():
    r = sr.Recognizer()  #recognizes voice
    with sr.Microphone() as source:
        audio=r.listen(source)  # listernsthe voice
    try:
...
def greeting():
    hour=int(time.strftime("%H"))   #extracts hour from time
